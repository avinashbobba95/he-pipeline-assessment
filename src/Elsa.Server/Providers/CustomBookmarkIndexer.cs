...
{
    public class CustomBookmarkIndexer : IBookmarkIndexer
    {
        ...
        public CustomBookmarkIndexer(
            ...
            ILogger<CustomBookmarkIndexer> logger)
        {
        ...
        public async Task IndexBookmarksAsync(IEnumerable<WorkflowInstance> workflowInstances,
            CancellationToken cancellationToken)
        {
            var workflowInstanceList = workflowInstances.ToList();

            foreach (var workflowInstance in workflowInstanceList.Where(x =>
                         x.WorkflowStatus == WorkflowStatus.Suspended))
        ...
        {
            _stopwatch.Restart();
            _logger.LogInformation("Indexing bookmarks for workflow instance {WorkflowInstanceId}",
                workflowInstance.Id);
            ...
            {
                _logger.LogWarning("Could not find workflow definition for workflow {WorkflowInstanceId}",
                    workflowInstance.Id);
                return;
            }

            var activityDefinition = workflowBlueprint.Activities.First();

            var blockingActivities = workflowBlueprint.GetBlockingActivities(workflowInstance!);
            if (!blockingActivities.Any())
            {
                var blockingActivity = new BlockingActivity(activityDefinition.Id, activityDefinition.Type);
                workflowInstance.BlockingActivities.Add(blockingActivity);
                blockingActivities = workflowBlueprint.GetBlockingActivities(workflowInstance!);
            }
            var bookmarkedWorkflows =
                await ExtractBookmarksAsync(workflowBlueprint, workflowInstance, blockingActivities, cancellationToken)
                    .ToList();
            ...
            _logger.LogDebug("Deleted {DeletedBookmarkCount} bookmarks for workflow {WorkflowInstanceId}",
                bookmarks.Count, workflowInstanceId);
            await _publisher.Publish(new BookmarkIndexingFinished(workflowInstanceId, bookmarks), cancellationToken);

            _stopwatch.Stop();
            _logger.LogInformation(
                "Indexed {BookmarkCount} bookmarks for workflow instance {WorkflowInstanceId} in {ElapsedTime}",
                entities.Count, workflowInstance.Id, _stopwatch.Elapsed);
        }
        ...
        public async Task DeleteBookmarksAsync(string workflowInstanceId, CancellationToken cancellationToken = default)
        {
            ...
            await _publisher.Publish(new BookmarksDeleted(workflowInstanceId, bookmarks), cancellationToken);
            _logger.LogDebug("Deleted {DeletedBookmarkCount} bookmarks for workflow {WorkflowInstanceId}", count,
                workflowInstanceId);
        }

        private async Task<IEnumerable<Bookmark>> FindBookmarksAsync(string workflowInstanceId,
        ...
        private IEnumerable<Bookmark> MapBookmarks(IEnumerable<BookmarkedWorkflow> bookmarkedWorkflows,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var workflowExecutionContext =
                new WorkflowExecutionContext(_serviceProvider, workflowBlueprint, workflowInstance);

            // Extract bookmarks for each blocking activity.
            var bookmarkedWorkflows = new List<BookmarkedWorkflow>();

            var activityTypes =
                (await _activityTypeService.GetActivityTypesAsync(cancellationToken)).ToDictionary(x => x.TypeName);

            foreach (var blockingActivity in blockingActivities)
            {
                if (activityTypes.ContainsKey(blockingActivity.Type))
                {
                    var activityExecutionContext = new ActivityExecutionContext(_serviceProvider,
                        workflowExecutionContext, blockingActivity, null, false, cancellationToken);
                    var activityType = activityTypes[blockingActivity.Type];
                    var providerContext = new BookmarkProviderContext(activityExecutionContext, activityType,
                        BookmarkIndexingMode.WorkflowInstance);
                    var providers = await FilterProvidersAsync(providerContext).ToListAsync(cancellationToken);

                    foreach (var provider in providers)
                    {
                        var bookmarkResults =
                            (await provider.GetBookmarksAsync(providerContext, cancellationToken)).ToList();
